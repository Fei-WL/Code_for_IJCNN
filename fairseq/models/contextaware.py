...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def build_encoder(cls, args, src_dict, embed_tokens):
        return ContextAwareEncoder(args, src_dict, embed_tokens)
...
class ContextAwareEncoder(FairseqEncoder):
    """
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        self.layers.extend(
            [self.build_encoder_layer(args) for _ in range(args.encoder_layers-1)]
        )
        self.context_layer = self.build_encoder_layer(args)

        self.context_attn = self.build_context_attention(args)
        self.context_attn_norm = LayerNorm(self.embed_dim)

        self.self_attn = self.build_self_attention(args)
        self.self_attn_norm = LayerNorm(self.embed_dim)
        ...
        self.gate_activate_fn = utils.get_activation_fn(
            activation=getattr(args, "activation_fn", "sigmoid")
        )
        self.gate_activate_dropout_module = FairseqDropout(
            float(activation_dropout_p), module_name=self.__class__.__name__
        )
    ...
    def build_context_attention(self, args):
    ...
    def forward(
        ...
        for layer in self.layers:
            curr = layer(curr, curr_encoder_padding_mask)

        for layer in self.layers:
            prev = layer(prev, prev_encoder_padding_mask)
        prev = self.context_layer(prev, prev_encoder_padding_mask)

        residual = prev
        if self.normalize_before:
            prev = self.context_attn_norm(prev)
        prev, _ = self.context_attn(
            query=prev,
            key=curr,
            value=curr,
        ...
        residual = curr
        if self.normalize_before:
            curr = self.self_attn_norm(curr)
        curr, _ = self.self_attn(
            query=curr,
            key=curr,
            value=curr,
            key_padding_mask=curr_encoder_padding_mask,
...
@register_model_architecture("contextaware", "contextaware")
