...
class ContextDecoderLayer(nn.Module):
    """Decoder layer block.
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def forward(
        ...
        context_out: Optional[torch.Tensor] = None,
        ...
        if context_out is not None and context_padding_mask is not None:
            residual = x
            if self.normalize_before:
                x = self.context_attn_layer_norm(x)

            x, attn = self.context_attn(
                query=x,
                key=encoder_out,
                value=encoder_out,
                key_padding_mask=encoder_padding_mask,
